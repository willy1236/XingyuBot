...
import discord
import numpy as np
from discord.commands import OptionChoice, SlashCommand, SlashCommandGroup
...
class command(Cog_Extension):
    ...
    def __init__(self, bot):
        super().__init__(bot)
        happycamp_guild_ids = [self.bot.happycamp_guild]
        self.role.guild_ids = happycamp_guild_ids
        self.party.guild_ids = happycamp_guild_ids
        self.registration_cmd.guild_ids = happycamp_guild_ids
        ...
        if user_list:
            for user in user_list.split():
                ...
                if user and user != self.bot.user:
                    ...
                    if ctx.guild.id == self.bot.happycamp_guild and not user.get_role(877934319249797120):
    ...
    async def save(self, ctx: discord.ApplicationContext):
        await ctx.defer()
        guild = self.bot.get_guild(self.bot.happycamp_guild)
        ...
        for role in guild.roles:
            ...
            try:
                ...
                log.info("新增： %s", role.name)
            except Exception as e:
                log.warning("儲存身分組時發生錯誤： %s", role.name, exc_info=e)
    ...
    async def update(self, ctx):
        ...
        guild_id = self.bot.check_registration(ctx.author)
        if guild_id:
            guild = self.bot.get_guild(guild_id)
            resgistration = self.bot.sqldb.get_registration_by_guildid(guild_id)
            role_guild = self.bot.get_guild(self.bot.happycamp_guild)
    ...
    async def batchset(self, ctx, role: discord.Option(discord.Role, name="要批量驗證的身分組")):
        ...
        for member in role.members:
            ...
            guild_id = self.bot.check_registration(member)
            if guild_id:
                guild = self.bot.get_guild(guild_id)
                resgistration = self.bot.sqldb.get_registration_by_guildid(guild_id)
                role = role_guild.get_role(resgistration.role_id)
