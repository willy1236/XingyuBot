            ...
            if monster.hp <= 0:
                ...
                if self.meet_monster.item_drops:
                    ...
                    for drop in self.meet_monster.item_drops:
                        if random.randint(1, 100) <= drop.drop_probability:
                            i = sqldb.get_player_item(player.discord_id, drop.item_id) or RPGPlayerItem(
                                item_id=drop.item_id, discord_id=player.discord_id, amount=0
                            )
        ...
        super().__init__(
            ...
            options=[
                discord.SelectOption(
                    label=i.customized_name if i.customized_name else i.template.name,
    ...
    async def button4_callback(self, button: discord.ui.Button, interaction: discord.Interaction):
        if interaction.user.id == self.user_dc.id and self.now_item:
            ...
            await interaction.response.edit_message(
                content=f"已售出未穿著的所有 {self.now_item.name} 並獲得 {total_price} Rcoin", embeds=[self.paginator.pages[self.paginator.current_page]]
            )
