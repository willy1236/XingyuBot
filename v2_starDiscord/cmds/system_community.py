...
from v2_starlib.database import APIType, Community, NotifyCommunityType, PlatformType
from v2_starlib.utils import nowtz
...
class system_community(Cog_Extension):
    ...
    async def twitch_set(
        ...
        twitch_user = self.bot.api.tw_api.get_user(twitch_user_login)
        if twitch_user:
            self.bot.sqldb.add_notify_community(type, twitch_user.id, guildid, channelid, roleid, msg, cache_time=nowtz())
            self.bot.sqldb.merge(Community(id=twitch_user.id, type=PlatformType.Twitch, display_name=twitch_user.display_name, username=twitch_user.login))

            type_tw = self.bot.Jsondb.get_tw(type.value, "twitch_notify_option")
            if role:
    ...
    async def twitch_remove(
        ...
        if not notify_type or notify_type == NotifyCommunityType.TwitchLive:
            self.bot.sqldb.remove_notify_community(NotifyCommunityType.TwitchLive, twitch_user.id, guildid)
        if not notify_type or notify_type == NotifyCommunityType.TwitchVideo:
            self.bot.sqldb.remove_notify_community(NotifyCommunityType.TwitchVideo, twitch_user.id, guildid)
        if not notify_type or notify_type == NotifyCommunityType.TwitchClip:
            self.bot.sqldb.remove_notify_community(NotifyCommunityType.TwitchClip, twitch_user.id, guildid)
    ...
    async def twitch_notify(self, ctx, twitch_user_login: discord.Option(str, required=True, name="twitch使用者名稱", description="英文的使用者名稱")):
        guildid = ctx.guild.id
        record = self.bot.sqldb.get_notify_community_user_byname(NotifyCommunityType.TwitchLive, twitch_user_login, guildid)
        if record:
    ...
    async def twitch_list(self, ctx: discord.ApplicationContext):
        ...
        dbdata = (
            self.bot.sqldb.get_notify_community_list(NotifyCommunityType.TwitchLive, guildid)
            + self.bot.sqldb.get_notify_community_list(NotifyCommunityType.TwitchVideo, guildid)  # pyright: ignore[reportOperatorIssue]
            + self.bot.sqldb.get_notify_community_list(NotifyCommunityType.TwitchClip, guildid)
        )  # type: ignore
    ...
    async def youtube_set(
        ...
        ytchannel = self.bot.api.google_api.get_channel(handle=ythandle)
        if ytchannel:
            self.bot.sqldb.add_notify_community(NotifyCommunityType.Youtube, ytchannel.id, guildid, channelid, roleid, msg, cache_time=nowtz())
            self.bot.sqldb.merge(Community(id=ytchannel.id, type=PlatformType.Google, display_name=ytchannel.snippet.title, username=ytchannel.snippet.customUrl))
        ...
        # TODO: 回歸定時任務
        record = self.bot.sqldb.get_push_record(ytchannel.id)
        if record.is_expired:
            config = self.bot.sqldb.get_websub_config(APIType.Google, 4)
            self.bot.api.yt_push.add_push(ytchannel.id, config.callback_uri, config.hub_secret)
            await asyncio.sleep(3)
            data = self.bot.api.yt_push.get_push(ytchannel.id, config.callback_uri, config.hub_secret)
    ...
    async def youtube_remove(self, ctx, ythandle: discord.Option(str, required=True, name="youtube帳號代碼", description="youtube頻道中以@開頭的代號")):
        ...
        self.bot.sqldb.remove_notify_community(NotifyCommunityType.Youtube, ytchannel.id, guildid)
    ...
    async def youtube_notify(self, ctx, ythandle: discord.Option(str, required=True, name="youtube帳號代碼", description="youtube頻道中以@開頭的代號")):
        ...
        record = self.bot.sqldb.get_notify_community_user_byid(NotifyCommunityType.Youtube, ytchannel.id, guildid)
        if record:
            channel = self.bot.get_channel(record.channel_id)
            role = channel.guild.get_role(record.role_id)
            if role:
    ...
    async def youtube_list(self, ctx):
        ...
        dbdata = self.bot.sqldb.get_notify_community_list(NotifyCommunityType.Youtube, guildid)
        for notify_data, community_data in dbdata:
    ...
    async def twitter_set(
        ...
        api_twitter_user = self.bot.api.cli_api.get_user_details(twitter_username)
        ...
        self.bot.sqldb.add_notify_community(NotifyCommunityType.TwitterTweet, api_twitter_user.id, guildid, channelid, roleid, None, cache_time=nowtz())
        self.bot.sqldb.merge(Community(id=str(api_twitter_user.id), type=PlatformType.Twitter, display_name=api_twitter_user.fullName, username=api_twitter_user.userName))
    ...
    async def twitter_remove(self, ctx, twitter_username: discord.Option(str, required=True, name="twitter使用者名稱", description="使用者名稱")):
        guildid = ctx.guild.id
        twitter_user = self.bot.api.cli_api.get_user_details(twitter_username)
        if not twitter_user:
            await ctx.respond(f"錯誤：查無此使用者 {twitter_username}")
            return
        self.bot.sqldb.remove_notify_community(NotifyCommunityType.TwitterTweet, twitter_user.id, guildid)
    ...
    async def twitter_notify(self, ctx, twitter_username: discord.Option(str, required=True, name="twitter使用者名稱", description="使用者名稱")):
        guildid = ctx.guild.id
        record = self.bot.sqldb.get_notify_community_user_byname(NotifyCommunityType.TwitterTweet, twitter_username, guildid)
        if record:
            channel = self.bot.get_channel(record.channel_id)
            role = channel.guild.get_role(record.role_id)
            if role:
    ...
    async def twitter_list(self, ctx):
        guildid = ctx.guild.id
        records = self.bot.sqldb.get_notify_community_list(NotifyCommunityType.TwitterTweet, guildid)
        if records:
