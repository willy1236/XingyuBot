...
from ..ui.view import McServerPanel, VIPAuditView, VIPView
...
class AnnoModal(discord.ui.Modal):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        embed.set_author(name="機器人全群公告", icon_url=self.Jsondb.get_picture("radio_001"))
        embed.set_footer(text="Bot Radio System")
        send_success = 0
        channels = self.sqldb.get_notify_channel_by_type(NotifyChannelType.AllAnnouncements)
...
class BotUpdateModal(discord.ui.Modal):
    def __init__(self, sqldb: SQLRepository, Jsondb: JsonDatabase):
        super().__init__()
    ...
    async def callback(self, interaction: discord.Interaction):
        msg = await interaction.response.send_message(f"訊息發送中..")
        embed = discord.Embed(description=self.children[0].value, color=0xC4E9FF, timestamp=datetime.now())
        embed.set_author(name="機器人更新通知", icon_url=self.Jsondb.get_picture("radio_001"))
        embed.set_footer(text="Bot Radio System")
        send_success = 0
        channels = self.sqldb.get_notify_channel_by_type(NotifyChannelType.BotUpdates)
...
class owner(Cog_Extension):
    ...
    @commands.is_owner()
    async def statue(self, ctx: discord.ApplicationContext, statue: str):
    ...
    async def cmd(self, ctx: discord.ApplicationContext, server_id=mcss_server_option, command=command_option):
        await ctx.defer()
        response = self.bot.api.mcss_api.excute_command(server_id, command)
        await ctx.respond(response if response else "指令已發送")
    ...
    async def findmember(self, ctx: discord.ApplicationContext, guildid: discord.Option(str, name="伺服器id")):
        ...
        guild_main = self.bot.get_guild(self.bot.happycamp_guild)
