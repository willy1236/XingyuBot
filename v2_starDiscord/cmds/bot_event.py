...
class event(Cog_Extension):
    ...
    async def on_ready(self):
        ...
        if bot.bot_code == "1" and not bot.debug_mode:
            # 移除過期邀請
            invites = await bot.get_guild(bot.happycamp_guild).invites()
        ...
        # 投票介面
        polls = bot.sqldb.get_active_polls()
        days_poll_period = timedelta(days=28)
        for poll in polls:
            if now - poll.created_at > days_poll_period:
                # 將超過28天的投票自動關閉
                poll.end_at = now
                bot.sqldb.merge(poll)
                log.debug("Ended poll: %s", poll.poll_id)
            else:
                bot.add_view(PollView(poll, sqldb=bot.sqldb, bot=bot), message_id=poll.message_id)
                log.debug("Loaded poll: %s", poll.poll_id)

        # 反應身分組
        for react_message in bot.sqldb.get_reaction_role_message_all():
            ...
            if message:
                react_roles = bot.sqldb.get_reaction_roles_by_message(react_message.message_id)
                ...
                bot.sqldb.delete_reaction_role_message(react_message.message_id)
                log.debug("Deleted reaction role message: %s", react_message.message_id)
        # 動態語音
        dynamic_voice_ids = bot.sqldb.get_all_dynamic_voice()
        if dynamic_voice_ids:
            ...
            bot.sqldb.batch_remove_dynamic_voice(removed_ids)

        # 私人頻道大廳
        ticket_lobbys = bot.sqldb.get_all_ticket_lobbys()
        ...
        # 私人頻道
        active_tickets = bot.sqldb.get_active_ticket_channels()
        ...
        # Bot status
        log.info(">> Bot online as %s <<", bot.user.name)
        log.info(">> Py-cord's version: %s <<", discord.__version__)
        if bot.debug_mode:
            ...
            activity_name = bot.sqldb.get_bot_activity(bot.bot_code)
        ...
        if total_cog_files == len(bot.cogs):
            ...
            log.warning(">> Cogs not all loaded, %d/%d loaded<<", len(bot.cogs), total_cog_files)
    ...
    async def on_message(self, message: discord.Message):
        ...
        if message.guild and message.guild.id == self.bot.happycamp_guild and not message.author.bot:
            # 貢丸防制
            if not is_owner:
                ...
                if result:
                    try:
                        ...
                        dbuser = self.bot.sqldb.get_dcuser(message.author.id)
    ...
    async def dynamic_room_trigger(self, member: discord.Member, before: discord.VoiceState, after: discord.VoiceState):
        # 動態語音
        if after.channel and (lobby_data := self.bot.sqldb.cache.dynamic_voice_lobby.get(after.channel.id)):
            # 新增
            ...
            overwrites = {
                target: perms for target, perms in after.channel.overwrites.items() if (isinstance(target, discord.Member) and me.top_role > target.top_role) or (isinstance(target, discord.Role) and me.top_role > target)
            }
            ...
            try:
                ...
                try:
                    ...
                    return
            self.bot.sqldb.add_dynamic_voice(new_channel.id, member.id, guild.id)
            try:
            ...
            if self.bot.get_channel(new_channel.id) and len(new_channel.members) == 0:
                try:
                    ...
                    self.bot.sqldb.remove_dynamic_voice(new_channel.id)
                except discord.errors.NotFound:
                    log.warning("動態語音頻道 %s 已經不存在", new_channel.id)
                except discord.errors.Forbidden:
                    log.warning("無法刪除動態語音頻道 %s", new_channel.id)
            return

        # 移除
        elif before.channel and not before.channel.members and self.bot.sqldb.getif_dynamic_voice_room(before.channel.id):
            try:
                await before.channel.delete(reason="動態語音：移除")
                self.bot.sqldb.remove_dynamic_voice(before.channel.id)
            except discord.errors.Forbidden:
    ...
    async def on_voice_state_update(self, member: discord.Member, before: discord.VoiceState, after: discord.VoiceState):
        ...
        # 語音進出紀錄
        voice_log_data = self.bot.sqldb.cache.voice_log.get(guildid)
        if voice_log_data is not None:
            NowTime = datetime.now()

            if before.channel:
                before_text = before.channel.mention if not self.bot.sqldb.getif_dynamic_voice_room(before.channel.id) else before.channel.name + " (動態語音)"
            else:
                before_text = ""
            if after.channel:
                after_text = after.channel.mention if not self.bot.sqldb.getif_dynamic_voice_room(after.channel.id) else after.channel.name + " (動態語音)"
            else:
                after_text = ""
    ...
    async def on_member_remove(self, member: discord.Member):
        ...
        dbdata = self.bot.sqldb.get_notify_channel(guildid, NotifyChannelType.MemberLeave)
        ...
        # 離開日誌
        notify_data = self.bot.sqldb.get_notify_channel(guildid, NotifyChannelType.LeaveLog)
        if notify_data:
    ...
    async def on_member_join(self, member: discord.Member):
        ...
        # 加入通知
        guildid = member.guild.id
        dbdata = self.bot.sqldb.get_notify_channel(guildid, NotifyChannelType.MemberJoin)
        ...
        if dbdata:
            ...
            if channel:
                ...
                log.warning("Member join channel: %s not found", dbdata.channel_id)

        # 加入日誌 / 警告系統：管理員通知
        notify_data = self.bot.sqldb.get_notify_channel(guildid, NotifyChannelType.JoinLog)
        if notify_data:
            log_channel_id = notify_data.channel_id
            dbdata = self.bot.sqldb.get_warnings_count(member.id)
            ...
            if channel:
                ...
                log.warning("Member join log channel: %s not found", log_channel_id)

        # 快樂營戶籍註冊
        if guildid == self.bot.happycamp_guild:
            earlest_guildid = self.bot.check_registration(member)
            if earlest_guildid and earlest_guildid != self.bot.happycamp_guild:
                from starlib.database.postgresql.models import DiscordUser

                dbdata = self.bot.sqldb.get_registration_by_guildid(earlest_guildid)
                user = DiscordUser(discord_id=member.id, registrations_id=dbdata.registrations_id)
                self.bot.sqldb.merge(user)
                await member.add_roles(member.guild.get_role(dbdata.role_id), reason="加入的最早伺服器")

    @commands.Cog.listener()
    async def on_guild_join(self, guild: discord.Guild):
        report_channel_obj = self.bot.get_channel(self.bot.settings.REPORT_CHANNEL)
        await report_channel_obj.send(f"公會異動：我加入了 {guild.name} ({guild.id})")

    @commands.Cog.listener()
    async def on_guild_remove(self, guild: discord.Guild):
        report_channel_obj = self.bot.get_channel(self.bot.settings.REPORT_CHANNEL)
        await report_channel_obj.send(f"公會異動：我離開了 {guild.name} ({guild.id})")

    @commands.Cog.listener()
    async def on_member_update(self, before: discord.Member, after: discord.Member):
        guildid = after.guild.id
        member = after
        if guildid == self.bot.happycamp_guild and before.nick != after.nick:
            p2 = re.compile(r"冠宇")
            nick = after.nick
    ...
    async def on_audit_log_entry(self, entry: discord.AuditLogEntry):
        if entry.guild.id == self.bot.happycamp_guild and entry.action == discord.AuditLogAction.member_update and entry.user != self.bot.user:
