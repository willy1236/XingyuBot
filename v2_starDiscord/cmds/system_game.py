...
from v2_starlib.database import LOLGameCache, LOLGameRecord, PlatformType, UserIPDetails
from v2_starlib.exceptions import APINetworkError
from v2_starlib.providers import *
...
def get_riot_account_puuid(bot: DiscordBot, user: discord.User, riot_id: str = None) -> str | None:
    user_game = bot.sqldb.get_user_game(user.id, PlatformType.LOL)
    if user_game:
        return user_game.other_id

    if riot_id:
        account = bot.api.riot_api.get_riot_account_byname(riot_id)
        if account:
...
class system_game(Cog_Extension):
    ...
    async def player(self, ctx, user: discord.Option(discord.Member, name="用戶", description="要查詢的用戶", default=None)):
        ...
        if player_data:
            ...
            for data in player_data:
                embed.add_field(name=self.bot.Jsondb.get_tw(data.game, "game_set_option"), value=data.player_name)
    ...
    async def lol_user(self, ctx, riot_id: discord.Option(str, name="riot_id", description="名稱#tag，留空則使用資料庫查詢", required=False)):
        ...
        account = self.bot.api.riot_api.get_riot_account_byname(riot_id)
        if not account:
            await ctx.respond("查詢失敗：查無此ID", ephemeral=True)
            return

        player = self.bot.api.riot_api.get_player_bypuuid(account.puuid)
        if player:
    ...
    async def match(self, ctx, matchid: discord.Option(str, name="對戰id", description="要查詢的對戰")):
        match = self.bot.api.riot_api.get_match(matchid)
        if match:
    ...
    async def playermatch(self, ctx, riot_id: discord.Option(str, name="riot_id", description="名稱#tag，留空則使用資料庫查詢", required=False)):
        puuid = get_riot_account_puuid(self.bot, ctx.author, riot_id)
        if not puuid:
            await ctx.respond("查詢失敗：查無此玩家", ephemeral=True)
            return

        match_list = self.bot.api.riot_api.get_player_matchs(puuid, 1)
        if not match_list:
            await ctx.respond("查詢失敗：此玩家查無對戰紀錄", ephemeral=True)
            return

        match = self.bot.api.riot_api.get_match(match_list[0])
        if match:
            await ctx.respond("查詢成功", embed=match.desplay())
        else:
    ...
    async def masteries(self, ctx, riot_id: discord.Option(str, name="riot_id", description="名稱#tag，留空則使用資料庫查詢", required=False)):
        ...
        masteries_list = self.bot.api.riot_api.get_summoner_masteries(puuid)
        if not masteries_list:
            await ctx.respond("查詢失敗：此玩家查無專精資料", ephemeral=True)

        player = self.bot.api.riot_api.get_riot_account_bypuuid(puuid)
    ...
    async def rank(self, ctx, riot_id: discord.Option(str, name="riot_id", description="名稱#tag，留空則使用資料庫查詢", required=False)):
        ...
        rank_data = self.bot.api.riot_api.get_summoner_rank(puuid)
        if rank_data:
    ...
    async def recentmatches(self, ctx, riot_id: discord.Option(str, name="riot_id", description="名稱#tag，留空則使用資料庫查詢", required=False)):
        ...
        match_list = self.bot.api.riot_api.get_player_matchs(puuid, 20)
        if not match_list:
            await ctx.respond("查詢失敗:此玩家查無對戰紀錄", ephemeral=True)
            return
    ...
    async def activematches(self, ctx, riot_id: discord.Option(str, name="riot_id", description="名稱#tag，留空則使用資料庫查詢", required=False)):
        ...
        active_match = self.bot.api.riot_api.get_summoner_active_match(puuid)
        if not active_match:
            player = self.bot.api.riot_api.get_riot_account_bypuuid(puuid)
    ...
    async def osu_map(self, ctx, mapid: discord.Option(str, name="圖譜id", description="要查詢的圖譜ID", default=None)):
        map = self.bot.api.osu_api.get_beatmap(mapid)
        if map:
            await ctx.respond("查詢成功", embed=map.display())
        else:
    ...
    async def apex_user(self, ctx: discord.ApplicationContext, username: discord.Option(str, name="玩家名稱", description="要查詢的玩家")):
        player = self.bot.api.apexapi.get_player(username)
        if player:
            await ctx.respond(content="查詢成功", embed=player.display())
        else:
    ...
    async def map(self, ctx):
        embeds = self.bot.api.apexapi.get_map_rotation().embeds()
    ...
    async def dbd_user(self, ctx, userid: discord.Option(str, name="steamid", description="要查詢的玩家id", default=None)):
        player = self.bot.api.dbd_interface.get_player(userid)
        if player:
    ...
    async def user(self, ctx, userid: discord.Option(str, name="用戶id", description="要查詢的用戶", default=None)):
        user = self.bot.api.steam_api.get_user(userid)
        if user:
    ...
    async def minecraft_set(self, ctx: RegisteredContext, username: discord.Option(str, name="玩家名稱", description="要設定的Minecraft玩家名稱，留空以刪除", default=None)):
        if not username:
            self.bot.api.sqldb.remove_external_account(ctx.cuser.id, PlatformType.Minecraft)
            await ctx.respond(content="已刪除Minecraft帳號資料", ephemeral=True)
            return

        user = self.bot.api.mojang_api.get_uuid(username)
        if user:
            self.bot.api.sqldb.upsert_external_account(ctx.cuser.id, PlatformType.Minecraft, user.id, username)
    ...
    async def zerotier(
        ...
        now = nowtz()
        account = UserIPDetails(ip=str(member["config"]["ipAssignments"][0]), last_seen=now, discord_id=ctx.author.id, address=member["nodeId"], name=name, registration_at=now)
        self.bot.sqldb.merge(account)
