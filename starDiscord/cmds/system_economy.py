...
class system_economy(Cog_Extension):
    ...
    async def check(self, ctx: discord.ApplicationContext, user: discord.Option(discord.Member, name="成員", description="留空以查詢自己", default=None)):
    ...
    async def give(
        ...
        elif given.bot is True:
            await ctx.respond(f"轉帳失敗：無法轉帳給機器人", ephemeral=True)
        else:
            code = self.bot.sqldb.transfer_scoin(giver.id, given.id, amount)
            if not code:
    ...
    async def set(
        ...
        user_point = self.bot.sqldb.get_coin(user.id)
        if mod == "add":
            user_point.stardust += amount
        elif mod == "set":
            user_point.stardust = amount
        self.bot.sqldb.merge(user_point)
    ...
    #     r = self.bot.sqldb.getif_coin(userid,2)
    #     if not r:
    #         await ctx.respond('你的星塵不足喔 需要2元才能遊玩')
    #         return

    #     self.bot.sqldb.update_coins(userid,'add',Coins.Stardust,-2)
    ...
    #             self.bot.sqldb.update_coins(userid,'add',Coins.Stardust,18)
    ...
    #     item = self.bot.sqldb.get_scoin_shop_item(item_id)
    #     if not item:
    #         await ctx.respond(f"{ctx.author.mention}：商店沒有賣這個喔")
    #         return

    #     buyer_id = self.bot.sqldb.getif_coin(ctx.author.id,item.price)
    #     if buyer_id:
    #         self.bot.sqldb.update_bag(ctx.author.id,item.item_uid,1)
