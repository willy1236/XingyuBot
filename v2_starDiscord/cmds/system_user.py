...
from v2_starlib.database.postgresql.models import DiscordUser

from ..checks import RegisteredContext, ensure_registered
from ..extension import Cog_Extension
from ..ui.embeds import BotEmbed
from ..ui.view import DeletePetView
...
class system_user(Cog_Extension):
    ...
    async def check(self, ctx, user_dc: discord.Option(discord.Member, name="用戶", description="可不輸入以查詢自己", default=None)):
        ...
        embed = pet.display(user_dc) if pet else BotEmbed.simple(f"{user_dc.name} 的寵物", "用戶沒有認養寵物")
    ...
    async def add(
        ...
        if r:
            ...
            await ctx.respond(f"你收養了一隻名叫 {name} 的{self.bot.Jsondb.get_tw(species, 'pet_option')}!")
