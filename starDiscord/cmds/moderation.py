...
class moderation(Cog_Extension):
    ...
    async def voice(
        ...
        if channel:
            ...
            if not channel.category.permissions_for(ctx.guild.me).manage_channels:
                ...
                await ctx.send(
                    embed=BotEmbed.simple("溫馨提醒", f"我無法在 {channel.category.mention} 中創建語音頻道，請確認我有足夠的權限（需求管理頻道）"),
                    delete_after=10,
                )
    ...
    async def ban(
        ...
        add_record: discord.Option(bool, name="是否要將此紀錄存入警告系統", description="將紀錄存入警告系統供其他群組檢視", default=False),
        delete_message_days: discord.Option(
            int, name="刪除指定天數內訊息", description="若提供，將刪除用戶指定天數內的所有訊息", default=None, min_value=1, max_value=7
        ),
    ):
        await ctx.defer()
