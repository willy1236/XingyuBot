...
from v2_starlib.database import APIType, DBCacheType, NotifyChannelType, NotifyCommunityType, UserIPDetails, UsersCountRecord, VoiceTime
from v2_starlib.providers.social.models import YoutubeVideo
from v2_starlib.utils import ensure_utc, get_arp_list, nowtz
...
class task(Cog_Extension):
    ...
    async def earthquake_check(self):
        ...
        timefrom = cache.value if cache else nowtz() - timedelta(days=1)
        try:
            earthquake_records = self.bot.api.cwa_api.get_earthquake_report_auto(timefrom.strftime("%Y-%m-%dT%H:%M:%S"), True)
    ...
    async def weather_check(self):
        weathers = self.bot.api.cwa_api.get_weather_data()
        if not weathers:
            return
        weather = weathers[0]
    ...
    async def weather_warning_check(self):
        apidatas = self.bot.api.cwa_api.get_weather_warning()
        if not apidatas:
            return

        cache = self.bot.sqldb.get_notify_cache(NotifyChannelType.WeatherWarning)
        report_time = cache.value if cache else nowtz() - timedelta(days=1)
    ...
    async def typhoon_warning_check(self):
        ...
        apidatas = self.bot.api.ncdr_rss.get_typhoon_warning(after=report_time)
        log.info("typhoon_warning_check: found %s new warnings, after %s", len(apidatas), report_time)
        if not apidatas:
            return
    ...
    async def apex_map_rotation(self):
        try:
            data = self.bot.api.apexapi.get_map_rotation()
            if data:
    ...
    async def twitch_live(self):
        ...
        data = self.bot.api.twitch_api.get_lives(users_id)
        log.debug("twitch_live data: %s", data)
    ...
    async def twitch_video(self):
        ...
        for user_id, cache in caches.items():
            videos = self.bot.api.twitch_api.get_videos(user_id, after=cache.value)
    ...
    async def twitch_clip(self):
        ...
        update_data: dict[str, datetime | None] = {}
        for user_id, cache in caches.items():
            clips = self.bot.api.twitch_api.get_clips(user_id, started_at=cache.value)
            if clips:
                ...
                if video_ids:
                    api_video = self.bot.api.twitch_api.get_videos(video_ids=list(set(video_ids)))
    ...
    async def youtube_video(self):
        ...
        for ytchannel_id, cache in caches.items():
            # 抓取資料
            rss_data = self.bot.api.yt_rss.get_videos(ytchannel_id, cache.value)
            if not rss_data:
                continue
    ...
    async def notify_twitter_tweet_updates(self):
        ...
        for twitter_user_id, cache in caches.items():
            try:
                log.debug("notify_twitter_tweet_updates: %s", twitter_user_id)
                # tweets = rss_hub.get_twitter(user_name, local=True, after=cache.value)
                results = self.bot.api.cli_api.get_user_timeline(twitter_user_id, after=cache.value)

                log.debug("notify_twitter_tweet_updates data: %s", results)

                if results is None:
                    log.warning("notify_twitter_tweet_updates error / not found: %s", twitter_user_id)
                ...
                log.exception("notify_twitter_tweet_updates error (id: %s): ", twitter_user_id)
                continue
    ...
    async def add_voice_time(self):
        ...
        for guild_id in voice_time_counter:
            ...
            if not guild:
                log.warning("Guild not found for voice time counting: %s", guild_id)
                continue
    ...
    async def refresh_yt_push(self):
        ...
        for record in self.bot.sqldb.get_expiring_push_records():
            # TODO: 需要處理 secret
            self.bot.yt_push.add_push(record.channel_id, config.callback_uri, config.hub_secret)
            await asyncio.sleep(3)
            data = self.bot.yt_push.get_push(record.channel_id, config.callback_uri, config.hub_secret)
            ...
            if data and data.has_verify:
                ...
                log.warning("refresh_yt_push failed: %s", record.channel_id)

    async def youtube_start_live_notify(self, video: YoutubeVideo):
        log.info("youtube_start_live_notify: %s", video.snippet.title)
        for _ in range(30):
            video_now = self.bot.api.google_api.get_video(video.id)[0]
    ...
    async def refresh_ip_last_seen_nmap(self):
        ...
        now = nowtz()
        ips = self.bot.sqldb.get_active_ips(ensure_utc(datetime(2025, 8, 11, 0, 0, 0)))  # 現在設定指定時間，之後再改成動態計算過去30天
    ...
    async def record_users_count(self):
        ...
        record = UsersCountRecord(record_date=nowtz().date(), users_count=users_count, servers_count=servers_count, shard_id=shard_id)
