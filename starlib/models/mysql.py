...
from sqlalchemy import BigInteger, Boolean, Column, ForeignKey, ForeignKeyConstraint, Identity, Integer, Interval, SmallInteger, String, Text, Date
...
class IPLastSeen(AlchemyBasicSchema):
    ...
    ip: str = mapped_column(String, primary_key=True)
    last_seen: datetime = mapped_column(TIMESTAMP(True, 0))
    discord_id: int | None = mapped_column(BigInteger)
    mac: str | None = mapped_column(String)
