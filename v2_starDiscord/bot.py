...
class DiscordBot(discord.Bot):
    ...
    async def error(self, ctx: discord.ApplicationContext, error: str) -> None:
        error_report_channel = self.get_channel(self.settings.ERROR_REPORT)
        embed = BotEmbed.general(name=str(ctx.author), icon_url=ctx.author.display_avatar.url, title="❌錯誤回報")
    ...
    async def report(self, msg: str, refer_msg: discord.Message | None = None):
        report_channel = self.get_channel(self.settings.REPORT_CHANNEL)
    ...
    async def feedback(self, ctx: discord.ApplicationContext, msg: discord.Message):
        feedback_channel = self.get_channel(self.settings.FEEDBACK_CHANNEL)
    ...
    async def dm(self, msg: discord.Message):
        dm_channel = self.get_channel(self.settings.DM_CHANNEL)
    ...
    async def mentioned(self, msg: discord.Message):
        dm_channel = self.get_channel(self.settings.MENTIONED_CHANNEL)
    ...
    async def mention_everyone(self, msg: discord.Message):
        dm_channel = self.get_channel(self.settings.MENTION_EVERYONE_CHANNEL)
