...
class DiscordBot(discord.Bot):
    ...
    async def error(self, ctx: discord.ApplicationContext, error: str) -> None:
        ...
        if ctx.command:
            embed.add_field(
                name='使用指令', value=f'```{ctx.command}```', inline=False)
            embed.add_field(
                name='參數', value=f'```{ctx.selected_options}```', inline=False)
        embed.add_field(
            name='使用者', value=f"{ctx.author}\n{ctx.author.id}", inline=False)
        embed.add_field(
            name='發生頻道', value=f'{ctx.channel}\n{ctx.channel.id}', inline=True)
        embed.add_field(
            name='發生群組', value=f'{ctx.guild}\n{ctx.guild.id}', inline=True)
    ...
    async def feedback(self, ctx: discord.ApplicationContext, msg: discord.Message):
        ...
        embed.add_field(
            name='發送者', value=f"{ctx.author}\n{ctx.author.id}", inline=False)
        embed.add_field(
            name='來源頻道', value=f'{ctx.channel}\n{ctx.channel.id}', inline=True)
        embed.add_field(
            name='來源群組', value=f'{ctx.guild}\n{ctx.guild.id}', inline=True)
    ...
    async def mentioned(self, msg: discord.Message):
        ...
        embed.add_field(
            name='發送者', value=f"{msg.author}\n{msg.author.id}", inline=False)
        embed.add_field(
            name='來源頻道', value=f'{msg.channel}\n{msg.channel.id}', inline=True)
        embed.add_field(
            name='來源群組', value=f'{msg.guild}\n{msg.guild.id}', inline=True)
        embed.timestamp = msg.created_at
        await dm_channel.send(embed=embed)

    async def mention_everyone(self, msg: discord.Message):
        dm_channel = self.get_channel(
            Jsondb.config.get('mention_everyone_channel'))
        embed = BotEmbed.general(name=msg.author, icon_url=msg.author.display_avatar.url,
    ...
    async def send_notify_communities(self,
                                      embed: discord.Embed,
