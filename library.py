...
class BRS():
    async def error(self, ctx, error: str):
        ...
        embed.add_field(name='使用指令',
                        value=f'```{ctx.message.content}```',
                        inline=False)
        embed.add_field(name='使用者',
                        value=f"{ctx.author}\n{ctx.author.id}",
                        inline=False)
        embed.add_field(name='發生頻道',
                        value=f'{ctx.channel}\n{ctx.channel.id}',
                        inline=True)
        embed.add_field(name='發生群組',
                        value=f'{ctx.guild}\n{ctx.guild.id}',
                        inline=True)
    ...
    async def feedback(self, ctx, msg):
        ...
        embed.add_field(name='發送者',
                        value=f"{ctx.author}\n{ctx.author.id}",
                        inline=False)
        embed.add_field(name='來源頻道',
                        value=f'{ctx.channel}\n{ctx.channel.id}',
                        inline=True)
        embed.add_field(name='來源群組',
                        value=f'{ctx.guild}\n{ctx.guild.id}',
                        inline=True)
    ...
    def all_anno(msg):
        ...
        embed.set_author(name="Bot Radio Station",
                         icon_url=picdata['radio_001'])
    ...
    def simple(description: str = discord.Embed.Empty,
               title: str = discord.Embed.Empty):
        '''簡易:不帶名稱'''
        embed = discord.Embed(title=title,
                              description=description,
    ...
    def lottery():
        '''Lottery System格式'''
        embed = discord.Embed(color=0xc4e9ff)
        embed.set_author(name="Lottery System",
                         icon_url=picdata['lottery_001'])
        return embed
