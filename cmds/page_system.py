...
class page_system(commands.Cog):
    ...
    async def pagetest_target2(self, ctx: commands.Context):
        paginator = pages.Paginator(pages=self.get_pages(2), use_default_buttons=False)
        paginator.add_button(pages.PaginatorButton("prev", label="test", style=discord.ButtonStyle.green))
        await paginator.send(ctx, target=ctx.channel, target_message="Paginator sent!")
